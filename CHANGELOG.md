...
### Changed
- **Breaking:** `--evm-network` no longer defaults to `arbitrum-one`; wallet and paid operations now require an explicit `--evm-network arbitrum-one|arbitrum-sepolia|local`. The default silently overrode a devnet manifest's EVM config, producing no-op mainnet-vault transactions on other chains that spent gas and set a useless ANT allowance before every chunk PUT failed payment verification. A preset selected alongside a manifest that carries an EVM block now also prints a warning that the manifest's EVM config is ignored. Read-only operations (`file download`, `chunk get`, node management) are unaffected.
