...
## Bootstrap peers, and the install-scripts problem

`postinstall.js` copies `bootstrap_peers.toml` into the platform config directory, mirroring
`install.sh` and `install.ps1` exactly — same destination, and only when the file is absent, so a
user's edited peer list is never overwritten.
...
npm. `ant-core` only ever *reads* that file, so without a fallback this would leave a CLI that
installs cleanly, reports its version, and then fails every network command — in exactly the
locked-down sandboxes this package exists to serve.

The binary already carries the same seeds: `ant-core/src/network_defaults.rs` embeds
`ant-core/resources/bootstrap_peers.toml` with `include_str!` and exposes
`bundled_bootstrap_seeds()`, which the WASM build relies on. `config::resolve_bootstrap_multiaddrs`
falls back to those seeds when there is no config file, instead of returning
`Error::NoBootstrapPeers`.

Priority is unchanged for every existing install: explicit `-b` peers beat a devnet manifest,
which beats the config file, which beats the bundled seeds. The fallback is reached only where
the alternative was a hard error, so it can never override a user's choice, and an explicitly
selected devnet manifest still errors rather than silently reaching for mainnet peers.

The postinstall is kept regardless, because a real file in the config directory is discoverable
and editable in a way a compiled-in constant is not.
