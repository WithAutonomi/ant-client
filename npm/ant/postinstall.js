...
//
// ant-core only ever reads this file, never writes it. It is not required, though: npm 12
// blocks install scripts by default (and `--ignore-scripts` does the same on older npm), so this
// frequently does not run, and config::resolve_bootstrap_multiaddrs falls back to the seeds
// compiled into the binary rather than failing with NoBootstrapPeers. What this script adds is a
// real file the user can find and edit. See npm/README.md.
//
