...
///
/// There is deliberately no default network: payments are irreversible
/// on-chain operations, and defaulting to mainnet silently discarded a
/// devnet manifest's EVM config — the mainnet vault address doesn't
/// exist on other chains, so payments "succeeded" as no-op transactions
/// and every subsequent chunk PUT failed verification (V2-471). Callers
/// must pass an explicit selection or get
/// [`Error::EvmNetworkNotSpecified`].
pub fn resolve_evm_network(
    name: Option<&str>,
    manifest: Option<&DevnetManifest>,
) -> Result<EvmNetwork> {
    match name {
        None => Err(Error::EvmNetworkNotSpecified),
...
mod tests {
    ...
    #[test]
    fn resolve_evm_network_requires_explicit_selection() {
        assert!(matches!(
            resolve_evm_network(None, None),
            Err(Error::EvmNetworkNotSpecified)
        ));
        // A manifest with an EVM block does not change that: no silent magic.
        assert!(matches!(
