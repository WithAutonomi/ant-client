...
/// Native Merkle vault transaction exposed to an external browser wallet.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct MerklePaymentRequest {
    /// Canonical vault calldata generated by evmlib.
    pub calldata: String,
    /// Maximum spend across possible winning pools, for token approval.
    pub maximum_amount: String,
    /// Expected tree depth in the confirmed event.
    pub depth: u8,
    /// Expected payment timestamp in the confirmed event.
    pub timestamp: u64,
    /// Eligible winning pool hashes.
    pub pool_hashes: Vec<String>,
}

impl MerklePaymentRequest {
    /// Build a wallet request from the ordinary native prepared batch.
    pub fn from_batch(batch: &crate::data::client::merkle::PreparedMerkleBatch) -> Self {
        Self {
            calldata: format!(
                "0x{}",
                hex::encode(
                    ant_protocol::evm::contract::payment_vault::encode_merkle_payment(
                        batch.depth,
            ...
            maximum_amount: ant_protocol::evm::Network::default()
            ...
            pool_hashes: batch
                .pool_commitments
                .iter()
                .map(|pool| hex::encode(pool.pool_hash))
                .collect(),
        }
    }
}

/// JSON representation of an EVM transaction log.
#[derive(Deserialize)]
pub struct PaymentLog {
    ...
    /// Encoded event fields.
    pub data: String,
}

/// Confirmed Merkle event, decoded with the native vault ABI.
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub struct MerkleSettlement {
    /// Selected pool from the prepared request.
    pub winner_pool_hash: String,
    /// Actual token spend.
    pub total_amount: String,
}

/// Validate a receipt's vault event against the prepared Merkle request.
pub fn decode_merkle_receipt(
    request: &MerklePaymentRequest,
    vault: &str,
    logs: &[PaymentLog],
) -> Result<MerkleSettlement, StorageQuoteError> {
    let vault = normalize_hex(vault, 20).map_err(StorageQuoteError)?;
    for log in logs {
        if normalize_hex(&log.address, 20).map_err(StorageQuoteError)? != vault {
            continue;
        }
        let topics = log
            .topics
            .iter()
            .map(|topic| decode_hex_array(topic.strip_prefix("0x").unwrap_or(topic), "event topic"))
            .collect::<Result<Vec<[u8; 32]>, _>>()?;
        let data = decode_unbounded_hex(&log.data, "event data")?;
        let Ok(event) =
            ant_protocol::evm::contract::payment_vault::decode_merkle_payment_event(&topics, &data)
        else {
            continue;
        };
        let winner = hex::encode(event.winnerPoolHash);
        if event.depth != request.depth
            || event.merklePaymentTimestamp != request.timestamp
            || !request.pool_hashes.contains(&winner)
        {
            continue;
        }
        if event.totalAmount
            > parse_decimal_amount(&request.maximum_amount, "maximum Merkle payment")?
        {
            return Err(StorageQuoteError(
                "Merkle event exceeds authorized amount".into(),
            ));
        }
        return Ok(MerkleSettlement {
            winner_pool_hash: winner,
            total_amount: event.totalAmount.to_string(),
        });
    }
    Err(StorageQuoteError(
        "receipt has no matching Merkle payment event".into(),
    ))
}
